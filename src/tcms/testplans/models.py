...
class TestPlan(TCMSActionModel):
    ...
    def apply_subtotal(
        ...
        print(">>>> testplans apply_subtotal cls:{}  > queryset{}".format(cls, queryset))
        select = {}
        if cases_count:
        ...
        if select:
            print(">>>> testplans apply_subtotal select:{}".format(select))
            queryset = queryset.extra(select=select)  # nosec
        print(">>>> testplans apply_subtotal queryset{}".format(queryset))
        return queryset
